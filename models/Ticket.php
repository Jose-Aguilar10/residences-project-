<?php
    class Ticket extends Conectar{
        ...
        public function listar_ticket_x_usu($usu_id){
            $conectar= parent::conexion();
            parent::set_names(); /* sentencia sql para listar los ticket por usuario */
            $sql="SELECT 
                tm_ticket.tick_id,
                tm_ticket.usu_id,
                tm_ticket.cat_id,
        ...
        public function listar_ticket(){   /* Lista todos los tickets en el apartado soporte   */
            $conectar= parent::conexion();
            parent::set_names();
            $sql="SELECT 
                tm_ticket.tick_id,
        ...
        public function listar_ticketdetalle_x_ticket($tick_id){
            ...
            $sql="SELECT
                ...
                FROM 
                td_ticketdetalle
                INNER join tm_usuario on td_ticketdetalle.usu_id = tm_usuario.usu_id  /* tm usuario hace un vinculo con ticketdetalle  */
                WHERE 
                tick_id =?";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $tick_id);
        ...
        public function insert_ticketdetalle_cerrar($tick_id,$usu_id){
            $conectar= parent::conexion();
            parent::set_names();
                $sql="INSERT INTO td_ticket(tickd_id,tick_id,usu_id,tickd_descrip,fech_crea,est) VALUES (NUll,?,?,'Ticket Cerrado...',now(),'1')";
            $sql=$conectar->prepare($sql);
            $sql->bindValue(1, $tick_id);
            $sql->bindValue(2, $usu_id);
            $sql->execute();
